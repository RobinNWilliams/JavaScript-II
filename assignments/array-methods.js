...
let fullName = [];
let first=runners.forEach(function(i){ //took a long time to understand the syntax of the .forEach
...
let allCaps = runners.map((athlete) => {
    return athlete.last_name.toUpperCase();
...
let ticketPriceTotal = runners.reduce((don_Total, don_Amnt) => {
return don_Total + don_Amnt.donation;
}, 0);
console.log(ticketPriceTotal);//initially got a huge number
                            //had to figure out the zero, needed help with the syntax
